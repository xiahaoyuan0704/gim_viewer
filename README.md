...
- 使用 That Open + web-ifc 加载 IFC 几何，支持模型列表、点击拾取、高亮、相机定位和属性查看；带 IFC 的 GIM 会按 FileDevRelation 只叠加当前 IFC 关联的 DEV/PHM/MOD 原生设备细节。
...
8. 使用 That Open / web-ifc 加载 IFC；如果 GIM 包同时含有 DEV/PHM/MOD，则优先按 FileDevRelation 额外叠加当前 IFC 关联的原生设备细节，并完成相机定位。
